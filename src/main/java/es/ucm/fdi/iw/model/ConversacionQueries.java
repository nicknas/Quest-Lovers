...
public class ConversacionQueries {
	...
	public static Conversacion findConversacion(EntityManager entityManager, int id_user1, int id_user2) {
		try {
			...
			Conversacion c = entityManager.createQuery("from Conversacion m where (m.user1 = :id_user1 and m.user2 = :id_user2) or (m.user2 = :id_user1 and m.user1 = :id_user2)", Conversacion.class).setParameter("id_user1", u1).setParameter("id_user2", u2).getSingleResult();
