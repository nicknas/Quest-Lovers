...
var links_finales = 0;
	jQuery(document).on("click", "#buttonStart", function(){
		if (!jQuery("#nombre_historia").val() || !jQuery("#descripcion").val()){
			...
			jQuery("fieldset").append('<div class="form-group" id="finButton"><label class="col-md-4 control-label" for="buttonNext"></label><div class="col-md-4 col-md-offset-4"><input name="${_csrf.parameterName}" type="hidden" value="${_csrf.token}" /><button id="buttonNext" type="button" name="buttonNext" class="btn btn-info ">Siguiente</button></div></div>');
		}
	...
	jQuery(document).on("click", "#buttonNext", function(){
		...
		jQuery(".form-group").remove();
		jQuery("legend").text("Pregunta Inicial");
